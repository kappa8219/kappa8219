...
def contributed_repositories(login: str, token: str) -> list[tuple[str, int]]:
    ...
    return [(node["nameWithOwner"], 0) for node in nodes if node and node.get("nameWithOwner")]


def build_section(rows: list[tuple[str, int]], from_events: bool, limit: int = 10) -> str:
    if not rows:
...
def main() -> int:
    ...
    section = build_section(rows, from_events=from_events)
